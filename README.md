...
- **并发与重试**：使用 `asyncio` + 信号量控制并发，内置 3 次重试，可一定程度抵抗临时网络波动。
- **健壮性不足**：缺少单元/集成测试，`AsyncFirecrawl` 客户端在 `scrape_single_article` 中每次请求都创建且未显式关闭，可能导致连接池或文件句柄占用增加。
- **改进建议**：补充基础测试（如文件命名与断点续传逻辑），通过异步上下文管理器复用并显式关闭 `AsyncFirecrawl` 客户端，并为爬取调用设置超时、区分网络错误与内容错误的异常类型。
